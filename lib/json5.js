...
// marker on the same line.
            ...
            for (i = 0, l = delim.length, extra_skip = 0; ; i = offset + 1) {
                ...
                if (chkstr_e.length && chkstr_e[0] !== '\r' && chkstr_e[0] !== '\n') {
                    ...
                    //
                    // Note: however, we DO TOLERATE a comma field separator following on the same line:
                    if (!chkstr_e.match(/^,(?:\r?\n|\r\n?)/)) {
                        continue;
                    } else {
                        // trailing comma found: bump skip length accordingly
                        extra_skip = 1;
                        chkstr_e = chkstr_e.substring(1);
                    }
                }
                chkstr_s = input.substr(offset - 1, 1);
                if (chkstr_s !== '\r' && chkstr_s !== '\n') {
                    // we don't tolerate any whitespace leading the EOT marker: 
                    // it must be at the start of a new line!
                    continue;
                } else if (chkstr_s === '\n' && input.substring(offset - 2, 1) === '\r') {
                ...
                // and we jump over the EOT marker line:
                offset += l + extra_skip;
                if (chkstr_e.length === 0) {
                    // EOT marker was the last thing in the input: we've hit the end!
                } else if (chkstr_e[0] === '\r' && chkstr_e[1] === '\n') {
                    // found trailing CRLF: skip it in its entirety:
                    offset += 2;
                } else {
                    offset++;
                }
                ...
                lineNumber += lines.length + 3;    // add the skipped newlines at start and end of heredoc: 1 at start, two at the end.
                break;
            }
