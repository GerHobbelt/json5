...
const lexStates = {
    ...
    heredocContent () {
        ...
        console.error('heredoc start:', [heredocMarker, source, source.substr(pos)])
        for (;;) {
            let idx = source.indexOf(eot_ch0, pos);
            if (idx < 0) {
                // pos = source.length
                // return newToken('string', source)
            ...
            pos = idx;
            console.error('marker test at', [pos, heredocMarker, source.substr(pos)])
            ...
            console.error('marker test done', [pos, heredocMarker, i, len])
            if (i === len) {
                ...
                let eol_seen = false;
                // did we hit EOF?
                console.error('marker end at EOF test', [pos, heredocMarker, c, pos, source.length])
